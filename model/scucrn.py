...
class ConvTransBlock(nn.Module):
    def __init__(self, conv_dim, trans_dim, head_dim, window_size, drop_path, type='W', input_resolution=None):
        ...
        self.conv1_1 = nn.Conv2d(
            self.conv_dim+self.trans_dim, self.conv_dim+self.trans_dim, 1, 1, 0, bias=True)
    ...
    def forward(self, x):
        conv_x, trans_x = torch.split(self.conv1_1(
            x), (self.conv_dim, self.trans_dim), dim=1)
...
class SCUCRN(nn.Module):
    def __init__(self, in_nc=3, config=[2, 2, 2, 2, 2, 2, 2], dim=64, drop_path_rate=0.0, input_resolution=256,STN=True,mask=False):
        super(SCUCRN, self).__init__()
        in_planes = 3
        if mask:
            in_planes = 4
        self.config = config
    ...
    def forward(self, x0):
        ...
        if self.stn and self.training:
            _, ctrl_points_x = self.stn_head(x0)
            x0, _ = self.tps(x0, ctrl_points_x)
